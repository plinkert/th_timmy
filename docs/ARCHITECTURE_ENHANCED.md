...
- **Playbook Structure (Step 1.1) and Query Generator (Step 1.2)**: Playbook structure with `technique_description`, `data_sources`, `output`, `triage`, and `response` in metadata.yml. Location: `automation_scripts.playbooks` and `playbooks/`. Main components: `playbook_validator` (validates technique_description, data_sources, hunting_indicators, TP/FP; rejects playbooks without required fields), `query_loader` (loads .sql, .json, .kql or .yml/.yaml from `queries/`; YAML format uses `query_id`/`query_ids`), `query_generator` (Step 1.2: generates ready-to-use query files from playbooks; loads via query_loader, filters by tool and mode, substitutes placeholders, saves to `queries_generated/`). Five example playbooks (T1055, T1059, T1562, T1082, T1486) use YAML format: `queries/elk.yml`, `queries/ms_defender.yml`. Sections: `output` (required columns for SIEM/EDR), `triage` (quick verification steps), `response` (on_confirmed, ir_playbook_ref, artifacts_to_collect). Depends on Step 0.2 (playbooks are synced to VMs). See [docs/PLAYBOOKS.md](PLAYBOOKS.md) for format and examples.
...
│   ├── normalizers/
│   └── utils/
├── playbooks/                 # MITRE ATT&CK playbooks (template + T1055…T1486); queries/elk.yml, ms_defender.yml
